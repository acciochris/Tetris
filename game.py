...
class Game:
    def __init__(self, blocks):
        ...
        self.canvas = AsyncCanvas(
            self.tk, width=1500, height=1000, highlightthickness=0
        )
        self.canvas.pack()
        self.tk.update()
        self.running = False
...
class Player:
    ...
    async def play(self):
        ...
        await canvas.create_text(
            *self.score_window_offset[self.window], text=self.name, font=("Calibri", 40)
        )
        self.score = await canvas.create_text(
            offset[0], offset[1] + 75, text="0", font=("Calibri", 40)
        )
        ...
        for a in range(len(next_block.shape)):
            for b in range(len(next_block.shape[a])):
                if next_block.shape[a][b]:
                    ...
                    x2 = x1 + 45
                    y2 = y1 + 45
                    id = await canvas.create_rectangle(
                        x1, y1, x2, y2, fill=next_block.color
                    )
                    ids.append(id)
        return ids
            ...
            for a in range(len(current_blocks[self.window].shape)):
                for b in range(len(current_blocks[self.window].shape[a])):
                    if current_blocks[self.window].shape[a][b]:
                        ...
                        y2 = y1 + 45
                        id = (
                            await canvas.create_rectangle(
                                x1, y1, x2, y2, fill=current_blocks[self.window].color
                            ),
                            (b + x, a + y),
                        )
            ...
            if game.running and current_ids[self.window] and self.dropping:
                ...
                for id, (x, y) in current_ids[self.window]:
                    ...
                    if windows[self.window].get((x, y + 1)) or y == 19:
                        ...
                        self.dropping = False
                        self.final_pos = {
                            (x, y): id for (id, (x, y)) in current_ids[self.window]
                        }
                        break
                else:
                    # drop one step
                    for id, (x, y) in current_ids[self.window]:
                        canvas.move(id, 0, 45)
                    current_ids[self.window] = [
                        (id, (x, y + 1)) for (id, (x, y)) in current_ids[self.window]
                    ]
            else:
                await sleep(0.005)
    ...
    async def show_score(self):
        ...
        offset = self.score_window_offset[self.window]
        self.score = await canvas.create_text(
            offset[0], offset[1] + 75, text=str(self.lines), font=("Calibri", 40)
        )
...
async def decide_winner():
    ...
    while True:
        ...
        if all(done):
            ...
            text = await canvas.create_text(
                700, 500, text=msg.format(winner), font=("Calibri", 50)
            )
            game.running = False
            await sleep(10)
            loop.stop()
